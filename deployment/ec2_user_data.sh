#!/bin/bash
# EC2 User Data script for automatically setting up the AWS Job Search application
# This script can be used in the UserData field when launching an EC2 instance
...
sudo -u postgres psql -c "CREATE USER aws_job_admin WITH PASSWORD 'change-this-password';"
...
DATABASE_URL=postgresql://aws_job_admin:change-this-password@localhost/aws_job_search
EOF
